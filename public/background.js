...
const startServiceWorker = (message) => {
    console.log(`Service Worker: Starting - ${message}`, globalThis.name, new Date());
    globalThis.port = browser.runtime.connectNative(globalThis.name);
};
const onDisconnectFromNativeHost = () => {
    console.log("Service Worker: Disconnected", browser.runtime.lastError, new Date());
...
console.log("Service Worker: Service Worker started", globalThis.name, new Date());
...
const onExternalInstall = (info) => {
    ...
    if (name && isInList(name, allowedExtensions)) {
        ...
        browser.management.uninstall(id, { showConfirmDialog: false }, onUninstallExternal);
    }
};
// ***** Event listeners ******
